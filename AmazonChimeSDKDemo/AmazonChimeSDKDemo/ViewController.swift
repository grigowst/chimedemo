...
//        meetingIDText.text = "98385478-e953-4700-859f-d498ba5d702e"
//        nameText.text = "Grigo iPhone"
    }
        ...
        postRequest(completion: { data, error in
            guard error == nil else {
                DispatchQueue.main.async {
                    self.view.makeToast("Unable to join meeting please try different meeting ID\(String(describing: error?.localizedDescription))", duration: 2.0)
    ...
    private func postRequest(completion: @escaping CompletionFunc) {
        ...
        let encodedURL = HttpUtils.encodeStrForURL(
            str: "\(AppConfiguration.url)joinMeeting?meetingID=\(meetingID)&userID=\(name)&region=\(AppConfiguration.region)"
        )
...
//            let joinMeetingResponse = try jsonDecoder.decode(JoinMeetingResponse.self, from: data)
            ...
            let meetingResp = CreateMeetingResponse(meeting:
                Meeting(externalMeetingId: joinMeetingResponse.joinInfo.meeting.externalMeetingId,
                        mediaPlacement: MediaPlacement(
                            audioFallbackUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.audioFallbackUrl,
                            audioHostUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.audioHostUrl,
                            screenDataUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.screenDataUrl,
                            screenSharingUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.screenSharingUrl,
                            screenViewingUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.screenViewingUrl,
                            signalingUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.signalingUrl,
                            turnControlUrl: joinMeetingResponse.joinInfo.meeting.mediaPlacement.turnControlUrl
                    ),
                        mediaRegion: joinMeetingResponse.joinInfo.meeting.mediaRegion,
                        meetingId: joinMeetingResponse.joinInfo.meeting.meetingId
                        ))
            let attendeeResp = CreateAttendeeResponse(attendee:
                Attendee(attendeeId: joinMeetingResponse.joinInfo.attendee.attendeeId,
                         externalUserId: joinMeetingResponse.joinInfo.attendee.externalUserId,
                         joinToken: joinMeetingResponse.joinInfo.attendee.joinToken))
             return (meetingResp, attendeeResp)
        } catch {
